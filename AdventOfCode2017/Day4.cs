...
inc mpys mzqmcwx vryz ibqrzc pmsy fat rojpxwy rcbqzi gjef";
        ...
        private static void Day4(string input)
        {
            var passPhrases = input.Split(Environment.NewLine);
            Day4(passPhrases);
        }

        private static void Day4(IEnumerable<string> passPhrases)
        {
            var passPhrasesList = passPhrases as IList<string> ?? passPhrases.ToList();

            // Part 1
            {
                var validPassPhrases = new List<string>();
                foreach (var passPhrase in passPhrasesList)
                {
                    var words = passPhrase.Split(" ");
                    if (words.SequenceEqual(words.Distinct()))
                        validPassPhrases.Add(passPhrase);
                }

                Console.WriteLine($"Number of valid passphrases for part 1: {validPassPhrases.Count}");
            }
            ...
            // Part 2
            {
                ...
                {
                    var words = passPhrase.Split(" ");
                    var wordsChars = words.Select(w => w.ToCharArray()).ToList();

                    // Turned into a local function since there appears to be no way to break out of a nested loop.
                    // Could have done this differently but this is neat enough.
                    void AddIfNoAnagrams()
                    {
                        for (var i = 0; i < wordsChars.Count - 1; i++)
                        {
                            var iWordChars = wordsChars[i].ToList();
                            iWordChars.Sort();
                            for (var j = i + 1; j < wordsChars.Count; j++)
                            {
                                var jWordChars = wordsChars[j].ToList();
                                jWordChars.Sort();
                                if (iWordChars.SequenceEqual(jWordChars))
                                    return;
                            }
                        }

                        // No anagrams found if we made it here.
                        validPassPhrases.Add(passPhrase);
                    }
                    AddIfNoAnagrams();
                }

                Console.WriteLine($"Number of valid passphrases for part 2: {validPassPhrases.Count}");
            }
        }
    }
}
