...
uhcach (18)";
        ...
        private class RecursiveProgram
        {
            public static Dictionary<string, RecursiveProgram> AllPrograms { get; set; } = new Dictionary<string, RecursiveProgram>();
            ...
            public int Weight
            {
                ...
                {
                    if (_weight != 0)
                        throw new ReadOnlyException("Weight may only be set once.");
                    _weight = value;
                }
            }
            private int _weight;
            public HashSet<RecursiveProgram> SubPrograms { get; set; } = new HashSet<RecursiveProgram>();

            public RecursiveProgram(string name, int weight = 0)
            {
                Name = name;
                Weight = weight;

                if (AllPrograms.ContainsKey(Name))
                    throw new Exception("AllPrograms already contains that program.");
                AllPrograms.Add(Name, this);
            }

            public static RecursiveProgram GetOrCreate(string name, int weight = 0)
            {
                // Get or create our program.
                RecursiveProgram program;
                if (AllPrograms.ContainsKey(name))
                {
                    program = AllPrograms[name];
                    if (program.Weight == 0) program.Weight = weight;
                }
                else
                {
                    program = new RecursiveProgram(name, weight);
                }
                return program;
            }

            // Weight of this program and all subprograms.
            public int CarryWeight
            {
                get
                {
                    return Weight + SubPrograms.Sum(subProgram => subProgram.CarryWeight);
                }
            }

            public (RecursiveProgram Program, int WeightDifference) FindUnbalancedStack()
            {
                if (!SubPrograms.Any()) return default;

                // Calculate weights of each stack.
                var stacksByWeight = new Dictionary<int, HashSet<RecursiveProgram>>();
                foreach (var subProgram in SubPrograms)
                {
                    var stackWeight = subProgram.CarryWeight;
                    if (!stacksByWeight.ContainsKey(stackWeight))
                        stacksByWeight.Add(stackWeight, new HashSet<RecursiveProgram>());
                    stacksByWeight[stackWeight].Add(subProgram);
                }
                if (!stacksByWeight.Any()) return default;

                // Sort by the amount of programs each weight contains.
                var orderedStacks = stacksByWeight.OrderBy(s => s.Value.Count).ToList();
                var unbalancedStack = orderedStacks.FirstOrDefault();
                var mostBalancedStack = orderedStacks.LastOrDefault();
                if (unbalancedStack.Key == mostBalancedStack.Key) return default;

                // Get the weight difference and return.
                var weightDifference = mostBalancedStack.Key - unbalancedStack.Key;
                return (unbalancedStack.Value.FirstOrDefault(), weightDifference);
            }

            public (RecursiveProgram Program, int WeightDifference) FindUnbalancedProgram(int weightDifference = 0)
            {
                var unbalancedStack = FindUnbalancedStack();
                if (unbalancedStack.Program == null)
                    return (this, weightDifference);
                return unbalancedStack.Program.FindUnbalancedProgram(unbalancedStack.WeightDifference);
            }

        }

        private static void Day7(string input)
        {
            var lines = input.Split(Environment.NewLine).ToList();
            Day7(lines);
        }

        private static void Day7(IEnumerable<string> input)
        {
            var lines = input as IList<string> ?? input.ToList();
            var allSubPrograms = new HashSet<RecursiveProgram>();

            foreach (var line in lines.Select(x => x.Trim()))
            {
                // Parse each line.
                var parenthesisStart = line.IndexOf('(');
                var parenthesisEnd = line.IndexOf(')');
                var arrowEnd = line.IndexOf('>');

                var programName = line.Substring(0, parenthesisStart - 1);
                var programWeight = int.Parse(line.Substring(parenthesisStart + 1, parenthesisEnd - parenthesisStart - 1));
                var subPrograms = (arrowEnd > -1) ? line.Substring(arrowEnd + 2).Split(", ") : null;

                var program = RecursiveProgram.GetOrCreate(programName, programWeight);

                // Deal with any subprograms if any as our last act.
                if (subPrograms == null) continue;
                foreach (var subProgramName in subPrograms)
                {
                    var subProgram = RecursiveProgram.GetOrCreate(subProgramName);
                    program.SubPrograms.Add(subProgram);
                    allSubPrograms.Add(subProgram);
                }
            }

            var subProgramNames = allSubPrograms.Select(p => p.Name).ToHashSet();
            var allProgramNames = RecursiveProgram.AllPrograms.Keys.ToHashSet();
            var bottomProgramName = allProgramNames.Except(subProgramNames).FirstOrDefault();

            if (bottomProgramName == null)
                throw new Exception("No bottom program found. Aborting.");

            Console.WriteLine($"The bottom program is {bottomProgramName}.");

            // Part 2.
            var bottomProgram = RecursiveProgram.GetOrCreate(bottomProgramName);
            var unbalancedStack = bottomProgram.FindUnbalancedProgram();
            Console.WriteLine($"Unbalanced program is {unbalancedStack.Program.Name}, weight difference is {unbalancedStack.WeightDifference} "
                + $"for a needed weight of {unbalancedStack.Program.Weight + unbalancedStack.WeightDifference}.");
        }
    }
}
