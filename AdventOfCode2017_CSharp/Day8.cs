...
namespace AdventOfCode2017_CSharp
{
    internal static partial class Program
    {
        ...
        {
            const string input = @"t inc 245 if xq != 0
hi inc 119 if gf >= -5
w dec 529 if s == 0
p inc 19 if fi > -2
...
w inc -854 if jlg < -1808";
        ...
        private static void Day8(string input)
        {
            var lines = input.Split(Environment.NewLine).ToList();
            Day8(lines);
        }

        private static void Day8(IEnumerable<string> input)
        {
            ...
            foreach (var line in lines)
            {
                ...
                if (!registers.ContainsKey(split[0]))
                    registers.Add(split[0], 0);
                var register = registers[split[0]];

                // Conditional register.
                if (!registers.ContainsKey(split[4]))
                    registers.Add(split[4], 0);
                var conditionalRegister = registers[split[4]];

                // Prepare our value to alter the register if the condition is met.
                var registerAdd = split[1] == "dec" ? 0 - int.Parse(split[2]) : int.Parse(split[2]);

                // Parse our condition and add to the register if needed.
                var conditionInt = int.Parse(split[6]);
                switch (split[5])
                {
                    case "==":
                        if (conditionalRegister == conditionInt)
                    ...
                    case "<=":
                        ...
                        break;
                    default:
                        throw new Exception($"Unknown conditional value {split[5]}");
                }

                if (register > highestValue)
                    highestValue = register;
                registers[split[0]] = register;
            }

            var highestRegister = registers.OrderByDescending(r => r.Value).First();
            Console.WriteLine($"Highest register is {highestRegister.Key} with value of {highestRegister.Value}.");
            Console.WriteLine($"Highest value held by a register is {highestValue}.");
        }
    }
}
